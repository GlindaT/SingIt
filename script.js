...
function aplicarCadenaDeAudio(audioCtx, source) {
 ...
 lowPass.frequency.value = 1000; // <--- Corta el brillo vocal, ideal solo para
afinador
...
function aplicarCadenaDeAudioKaraoke(audioCtx, source) {
 ...
 compresor.threshold.setValueAtTime(-24, audioCtx.currentTime); // Límite de
volumen
 compresor.knee.setValueAtTime(30, audioCtx.currentTime); // Transición
suave
 compresor.ratio.setValueAtTime(4, audioCtx.currentTime); // Fuerza de
compresión
 compresor.attack.setValueAtTime(0.003, audioCtx.currentTime); // Reacción
inmediata (3ms)
 compresor.release.setValueAtTime(0.25, audioCtx.currentTime); // Relajación
natural (250ms)
...
function detectPitch() {
 ...
 guide.textContent = ` Estás grave. Sube a ${targetNote}`; ⬆
 guide.style.color = "#f59e0b";
 } else {
 display.style.color = "#f59e0b";
 guide.textContent = ` Estás agudo. Baja a ${targetNote}`; ⬇
 guide.style.color = "#f59e0b";
 }
 } else {
 display.textContent = "--";
...
async function startStudioRecording() {
 ...
 studioStream = await navigator.mediaDevices.getUserMedia({audio:
audioConstraints1});
 ...
 studioStream2 = await navigator.mediaDevices.getUserMedia({audio:
audioConstraints2});
...
mimeType: "audio/webm;codecs=opus" } : {};
 ...
 const mic1Name = mic1Select ?
mic1Select.options[mic1Select.selectedIndex]?.text : "Predeterminado";

 if (isDuo && mic2Id) {
 const mic2Select = $("mic2Select");
 const mic2Name = mic2Select ?
mic2Select.options[mic2Select.selectedIndex]?.text : "Mic 2";
 $("studioStatus").textContent = `Estado: Grabando DÚO (${mic1Name} + 🔴
${mic2Name})...`;
 } else {
...
function redoStudioRecording() {
 ...
 studioRecordedBlob = null;
 $("voicePlayer").src = "";
 $("studioStatus").textContent = "Estado: grabación eliminada. Lista para volver a
grabar.";
}
...
async function saveToLibrary(blob, options = {}) {
 ...
 textoPlano: options.textoPlano || null, // Compatibilidad con el nuevo
formato
...
async function renderLibrary(filter = 'todos') {
 ...
 const previewTexto = item.textoPlano ? item.textoPlano.substring(0, 120)
+ "..." : "Sin contenido";
 div.innerHTML = `
 <p><strong>${item.name}</strong></p>
 <small>Tipo: TEXTO ULTRASTAR | ${item.date}</small> 📝
 <div style="background: var(--bg-main); padding: 10px; border-radius:
6px; font-family: monospace; font-size: 12px; margin: 10px 0; white-space:
pre-wrap; border: 1px solid var(--border); color: var(--text-muted);">
 ${previewTexto}
 </div>
 <div style="display: flex; gap: 10px;">
 <button type="button" data-id="${item.id}" class="load-monitor-btn"
style="background:#3b82f6; color:white;"> Cargar en Monitor</button> 📥
 <button type="button" data-id="${item.id}" class="delete-library-btn"
style="background:#e11d48;"> Eliminar</button> 🗑
 </div>
 `;
 }
 // Si es cualquier otro archivo (pista, voz, grabación, karaoke con audio)
 else {
 // Validamos que exista el blob antes de crear la URL para evitar errores
accidentales
...
margin: 10px 0;"></audio>` : '<p style="color:red; font-size:12px;">Audio no
encontrado</p>'}
 <button type="button" data-id="${item.id}" class="delete-library-btn"
style="background:#e11d48;"> Eliminar</button> 🗑
 `;
 ...
 // CORRECCIÓN: Buscamos "lyricsText" (el ID real de tu monitor del 🎯
Estudio)
 const monitor = document.getElementById("lyricsText") ||
document.getElementById("miniMonitorTextArea");

 if (monitor) {
 monitor.value = item.textoPlano;

 // Si el archivo UltraStar contiene los tiempos de los taps guardados
en memoria, los reactivamos
...
renderKaraokeLyrics(transcriptionSegments);
 ...
 alert(` Letra de "${item.name}" cargada en el monitor del ✅
Estudio.`);

 // Scroll suave automático directo al monitor de texto para empezar a
trabajar
 monitor.scrollIntoView({ behavior: "smooth", block: "center" });
 } else {
 alert(" No se encontró el contenedor visual del monitor en esta ⚠
pantalla.");
 }
 }
 });
 ...
 if (typeof loadVoiceOptionsInStudio === "function") await
loadVoiceOptionsInStudio();
 if (typeof loadTrackOptionsInStudio === "function") await
loadTrackOptionsInStudio();
 if (typeof loadTrackOptionsInKaraoke === "function") await
...
async function loadTrackOptionsInStudio() {
 ...
 if (!select) return;
 select.innerHTML = `<option value="">Selecciona una pista desde
Biblioteca</option>`;
 try {
...
⚡)";
 ...
 status.textContent = `Estado: voz seleccionada -> ${item.name} (sin
...
async function transcribeSelectedVoice() {
 ...
 alert(" Primero selecciona y carga una voz desde Biblioteca"); ⚠
 return;
 }
 const status = $("selectedVoiceStatus");
 const lyricsText = $("lyricsText");
 ...
 if (status) {
 status.textContent = `Estado: Transcribiendo parte ${chunkNumber} de
${totalChunks}...`;
 }
 ...
 transcriptionSegments =
splitSegmentsIntoKaraokeLines(baseTranscriptionSegments, 6);
 renderKaraokeLyrics(transcriptionSegments);
 cargarLetrasEnMonitor();
 if (lyricsText) {
 ...
 const nombreBase = vozOriginal ? vozOriginal.name.replace(/ Voz - |Voz - 🎙
/g, "") : "Nueva Canción";
...
Transcribe\n#BPM:120\n#GAP:0\n`;
 ...
 console.error(" Error al generar el archivo UltraStar independiente:", ❌
err);
 }
...
async function guardarTextoUltraStarEnBiblioteca() {
 ...
 alert(" El monitor está vacío. No hay texto para guardar."); ⚠
 return;
 }
 // 2. Extraer metadatos básicos para el nombre (puedes usar variables globales
de tu app)
 ...
 type: "ultrastar_txt", // <--- Este es el nuevo tipo de archivo para el
filtro
...
function buildWordTimingFromSegment(segment) {
 ...
 const totalChars = rawWords.reduce((sum, word) => sum + word.length, 0) ||
rawWords.length;
...
async function analyzePitchForSegments(audioBlob, segments) {
 ...
 console.log(" No hay audio o segmentos para analizar"); ⚠
 return segments;
 }
 try {
 const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
...
function renderKaraokeLyrics(segments) {
 ...
 container.innerHTML = `<p class="karaoke-placeholder">No hay segmentos para
mostrar.</p>`;
 return;
 }
...
function cargarPistaKaraoke(e) {
 ...
 track.volume = 0.4;
 $("karaokeStatus").textContent = "Estado: Pista lista. ¡Presiona Iniciar
Grabación!";
 cargarLetrasEnMonitor();
}
async function loadTrackOptionsInKaraoke() {
 ...
 if (!select) return;
 select.innerHTML = `<option value="">Selecciona una pista desde tu
Biblioteca</option>`;
 try {
...
async function loadSelectedTrackFromLibraryKaraoke() {
 ...
 if (!id) {
 alert(" Selecciona una pista de la lista."); ⚠
 return;
 }
 try {
 ...
 track.volume = 0.4;
 $("karaokeStatus").textContent = `Estado: Pista cargada (${item.name}). ¡Inicia
grabación!`;
 cargarLetrasEnMonitor();
 } catch (error) {
 console.error(error);
...
function cargarLetrasEnMonitor() {
 ...
 if (!container) return;
 console.log("cargarLetrasEnMonitor -> transcriptionSegments:",
transcriptionSegments);
 container.innerHTML = "";
 if (!Array.isArray(transcriptionSegments) || transcriptionSegments.length === 0)
{
 container.innerHTML = `<p class="karaoke-placeholder"
style="font-size:18px;"> Ve a la pestaña 'Estudio', transcribe una voz y vuelve ⚠
aquí para ver la letra.</p>`;
 return;
 }
...
async function startKaraokeRecording() {
 const track = $("karaokeTrack");
 if (!track || !track.src) { alert(" Primero sube una pista instrumental."); ⚠
return; }
 try {
 const micCount = $("micCount");
 const isDuo = micCount && micCount.value === "2";
 // 1. LIMPIEZA ABSOLUTA DE HARDWARE ANTES DE EMPEZAR
 // Si había flujos abiertos del Afinador o Estudio, los apagamos para liberar
los canales
 if (window.karaokeStream && typeof window.karaokeStream.getTracks ===
'function') {
 window.karaokeStream.getTracks().forEach(t => t.stop());
 }
 if (window.karaokeStream2 && typeof window.karaokeStream2.getTracks ===
'function') {
 ...
 karaokeDuoAudioContext = new (window.AudioContext ||
window.webkitAudioContext)();
 ...
 // CORRECCIÓN 1: Declaramos una constante local limpia (stream1) para aislar 🔥
el hardware
 const stream1 = await navigator.mediaDevices.getUserMedia({ audio:
audioConstraints1 });
 window.karaokeStream = stream1; // Guardamos la referencia global para
detenerlo después si es necesario
 // Procesar Mic 1 de forma totalmente independiente
 const source1 = karaokeDuoAudioContext.createMediaStreamSource(stream1);
 const mic1Filtrado = aplicarCadenaDeAudioKaraoke(karaokeDuoAudioContext,
source1);
 ...
 const stream2 = await navigator.mediaDevices.getUserMedia({ audio:
audioConstraints2 });
 window.karaokeStream2 = stream2;
 const source2 = karaokeDuoAudioContext.createMediaStreamSource(stream2);
 const mic2Filtrado = aplicarCadenaDeAudioKaraoke(karaokeDuoAudioContext,
source2);
...
mic1Select.options[mic1Select.selectedIndex]?.text : "Predeterminado";
 ...
 const mic2Name = mic2Select ?
mic2Select.options[mic2Select.selectedIndex]?.text : "Mic 2";
 $("karaokeStatus").textContent = `Estado: Grabando DÚO (${mic1Name} + 🔴
${mic2Name})...`;
 } else {
...
async function mixKaraoke() {
 ...
 alert(" Faltan ingredientes: Asegúrate de cargar una pista instrumental y ⚠
grabar tu voz primero.");
 return;
 }
 ...
 btn.disabled = true;
 resultDiv.innerHTML = "<p style='color: var(--text-muted);'>Uniendo la pista y tu
voz. Esto puede tardar unos segundos...</p>";
 try {
 const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
 ...
 <audio controls src="${finalUrl}" style="width: 100%; margin-bottom: 15px;
border-radius: 8px;"></audio>
 <div style="display: flex; gap: 10px;">
 <a href="${finalUrl}" download="Mezcla_${trackFile.name || "Karaoke"}.wav"
style="flex: 1;">
 <button type="button" style="width: 100%; background: #22c55e; color:
black;"> Descargar Archivo</button> 💾
 </a>
 <button id="saveMixToLibBtn" type="button" style="flex: 1; background:
#3b82f6; color: white;"> Guardar en Biblioteca</button> 📁
 </div>
 `;
 ...
 resultDiv.innerHTML = "<p style='color: #ef4444;'> Hubo un error al mezclar ❌
los audios.</p>";
 } finally {
...
async function splitAudio() {
 ...
 const directUrl = tmpData.data.url.replace("tmpfiles.org/",
"tmpfiles.org/dl/");
 ...
 detailText.textContent = "Mezclando bajo, batería y melodía en una sola
pista instrumental...";
 const urls = statusData.output;
 let vocalUrl = null;
 let instUrls = [];
 if (Array.isArray(urls)) {
 urls.forEach(u => u.toLowerCase().includes("vocal") ? (vocalUrl = u) :
instUrls.push(u));
 if (!vocalUrl) {
 vocalUrl = urls[0];
...
window.webkitAudioContext)();
 ...
 const offlineCtx = new OfflineAudioContext(2, maxLength,
buffers[0].sampleRate);
 ...
 await saveToLibrary(blobPista, { name: `Pista - ${file.name}`, type:
"pista" });
 statusText.textContent = " ¡Separación perfecta!"; 🎉
 detailText.textContent = "Voz pura y Pista Instrumental guardadas en
Biblioteca.";
 btn.disabled = false;
...
"canceled") {
 ...
 detailText.textContent = `Estado de la IA: ${statusData.status}... por
favor espera.`;
 }
 } catch (pollError) {
 clearInterval(interval);
 ...
 detailText.textContent = pollError.message || "Revisa la consola para más
detalles.";
 btn.disabled = false;
...
function initSettings() {
 ...
 const todosLosTemas = ["theme-clasico", "theme-moderno", "theme-disco",
"theme-acustico", "theme-fiesta"];
 todosLosTemas.forEach(tema =>
contenedorKaraoke.classList.remove(tema));
...
async function loadAvailableMics() {
 ...
 if (mics.length === 0) {
 mic1Select.innerHTML = `<option value="">No se detectaron
micrófonos</option>`;
 } else {
 ...
 if (mics.length === 0) {
 mic2Select.innerHTML = `<option value="">No se detectaron
micrófonos</option>`;
 } else {
 mics.forEach((mic, index) => {
 const option = document.createElement("option");
 ...
 console.log(" Micrófonos detectados:", mics.length); 🎙
 } catch (error) {
 ...
 if (mic1Select) {
 mic1Select.innerHTML = `<option value=""> Permite acceso al ⚠
micrófono</option>`;
 }
 if (mic2Select) {
 mic2Select.innerHTML = `<option value=""> Permite acceso al ⚠
micrófono</option>`;
 }
 }
}
...
async function testMicrophone(micNumber) {
 ...
 if (!deviceId) {
 alert(" Selecciona un micrófono primero"); ⚠
 return;
 }
 try {
 const constraints = {
...
async function applyCorrectedLyrics() {
 ...
 if (!correctedText) {
 alert(" No hay texto corregido para aplicar."); ⚠
 return;
 }
 if (!Array.isArray(baseTranscriptionSegments) ||
!baseTranscriptionSegments.length) {
 alert(" Primero transcribe una voz antes de corregir la letra."); ⚠
 return;
 }
 ...
 alert(" No se pudo reconstruir la letra corregida."); ⚠
 return;
 }
 ...
 console.error(error);
 if (status) {
 status.textContent = "Estado: letra corregida aplicada, pero no se pudo
guardar en BD";
 }
 }
 } else {
...
function startTapSync() {
 ...
 alert(" Primero escribe o corrige la letra en el área de texto."); ⚠
 return;
 }

 if (!voicePlayer || !voicePlayer.src) {
 alert(" Primero carga una voz desde la Biblioteca."); ⚠
 return;
 }
 ...
 alert(" No hay líneas de texto para sincronizar."); ⚠
 return;
 }
 ...
 // Reproducir la PISTA desde el inicio (la sincronización es contra el
instrumental)
...
function updateTapSyncDisplay() {
 ...
 if (progressEl) {
 progressEl.textContent = `${tapSyncCurrentIndex} / ${tapSyncLines.length}
líneas`;
 }
}
...
async function applyTapSync() {
 ...
 alert(" No hay datos de sincronización."); ⚠
 return;
 }
 ...
 // Mostrar estado
 if (status) status.textContent = "Estado: Aplicando tiempos y analizando
notas...";
 ...
 let end = (i < tapSyncTimestamps.length - 1) ? tapSyncTimestamps[i + 1] :
(totalDuration || start + 3);
...
🎵";
 analyzedSegments = await analyzePitchForSegments(selectedVoiceBlob,
newSegments);
 }
 ...
 console.warn(" No hay pista instrumental seleccionada para crear ⚠
karaoke");
 }
 ...
 cargarLetrasEnMonitor();
 if (selectedVoiceId) {
 updateLibraryItem(selectedVoiceId, { transcription:
baseTranscriptionSegments })
 ...
 tapSyncTimestamps = [];
 tapSyncCurrentIndex = 0;
 if (status) status.textContent = "Estado: Sincronización y notas ✅
aplicadas";
...
document.addEventListener("DOMContentLoaded", async () => {
 ...
 if (btn) {
 btn.textContent = autoScrollEnabled ? " Auto-scroll: ON" : " 🔒 🔓
Auto-scroll: OFF";
...
function reconstruirFraseDesdeWords(segmento) {
 ...
 if (!segmento) return "";
 // 2. Intentar buscar el arreglo de palabras (revisamos .words y .text como plan
B)
 const listaPalabras = Array.isArray(segmento.words) ? segmento.words : [];

 // 3. PLAN B AUTOMÁTICO: Si .words está vacío, pero el segmento ya tiene un texto
plano general
 if (listaPalabras.length === 0 && segmento.text) {
 return segmento.text.trim();
 }
 // 4. Si encontramos palabras individuales, las unimos asegurando mapear la
propiedad correcta
 return listaPalabras
 .map(w => {
 // Extraemos el texto buscando cualquier propiedad común (.text, .word o el
objeto directo)
 let textoPalabra = "";
...
function drawKaraokeMonitor(currentTime, currentFreq, currentFreq2) {
 ...
 // ========================================================
 const temaActual = localStorage.getItem("singIt_karaoke_theme") ||
"theme-clasico";
 ...
 // Genera un color sutil animado basado en los milisegundos para simular luces
locas
 ...
 let isCorrect = false;
 if (isActive) {
 if (currentFreq > 0 && Math.abs(frequencyToMidi(currentFreq) - midi) <=
2) isCorrect = true;
 if (currentFreq2 > 0 && Math.abs(frequencyToMidi(currentFreq2) - midi) <=
2) isCorrect = true;
 }
...
"..";
 ...
 ctx.font = "16px Arial";
 ctx.textAlign = "center";
 ctx.fillText("Sincroniza una canción en 'Estudio' para ver las notas",
...
const width = ctx.canvas.width; // Tomamos el ancho dinámico del canvas
 ...
 // Volvemos a tu cálculo original en X pero asegurando el espaciado correcto
hacia atrás
 ...
 started1 = false; // Rompe el trazo de forma limpia si hay silencio para
evitar líneas locas
 }
 });
 ctx.stroke();
 ...
 // Desfasamos ligeramente a la derecha (X = 46) para que si cantan al unísono,
ambos círculos se vean
 ...
 ctx.fillText("Próximo: " + textoProximoLimpio, canvas.width / 2,
canvas.height - 8);
 }
 } else {
 const upcomingSegment = transcriptionSegments.find(seg => seg.start >
currentTime);
 if (upcomingSegment) {
...
async function startKaraokePitchDetection() {
 ...
 // 1. Calculamos el volumen real (RMS) de este buffer específico antes
de analizar
 let sum1 = 0;
 for (let i = 0; i < buffer1.length; i++) { sum1 += buffer1[i] *
buffer1[i]; }
 const rms1 = Math.sqrt(sum1 / buffer1.length);
 // 2. Filtro de confianza: Si el volumen es menor a 0.015, es ruido
eléctrico de fondo.
 // Ignoramos el cálculo del pitch (-1) para evitar que la esfera se
caiga al piso del canvas.
 if (rms1 > 0.015) {
 pitch1 = autoCorrelate(buffer1, karaokeDuoAudioContext?.sampleRate
|| 48000);
 } else {
 pitch1 = -1;
 }
 ...
 let sum2 = 0;
 for (let i = 0; i < buffer2.length; i++) { sum2 += buffer2[i] *
buffer2[i]; }
...
function parseUltrastarTxt(content) {
 ...
 const type = parts[0]; // : = normal, * = golden, F = freestyle, - = line
break
...
function ultrastarToSegments(parsed) {
 ...
 const beatDuration = 60 / bpm / 4; // Duración de un beat en segundos (UltraStar
usa quarter beats)
 ...
 const midiNote = 60 + note.pitch; // UltraStar usa pitch relativo, base = C4
(60)
...
async function loadKaraokeCatalog() {
 ...
 container.innerHTML = `<p style="color: var(--text-muted);">Cargando
catálogo...</p>`;
 ...
 <button type="button" class="load-catalog-btn"
data-folder="${song.folder}" data-title="${song.title}"
data-artist="${song.artist}" style="background: #22c55e;"> Cantar</button> ▶
 </div>
 `;
...
async function loadCatalogSong(folder, title, artist) {
 ...
 if (status) status.textContent = `Estado: "${title}" cargada. ¡Lista para
cantar! `; 🎤
...
async function loadMyKaraokeSongs() {
 ...
 const voces = await getLibraryItemsByType("voz");
 const vocesConSync = voces.filter(v => v.transcription &&
v.transcription.length > 0);
 ...
 <button type="button" class="load-karaoke-btn" data-id="${song.id}"
style="background: #22c55e;"> Cantar</button> ▶
 <button type="button" class="share-karaoke-btn" data-id="${song.id}"
style="background: #8b5cf6; padding: 8px 10px;" title="Compartir como
.singit"> </button> 📤
 <button type="button" class="delete-karaoke-btn" data-id="${song.id}"
style="background: #ef4444; padding: 8px 10px;"> </button> 🗑
 </div>
 `;
 ...
 btn.addEventListener("click", () =>
exportKaraokeSong(Number(btn.dataset.id)));
 });
 ...
 container.innerHTML = `<p style="color: #ef4444;">Error al cargar
canciones</p>`;
 }
}
async function loadKaraokeSong(id) {
 ...
 $("karaokeStatus").textContent = `Estado: "${title}" cargada. ¡Lista para
cantar! `; 🎤
...
async function exportKaraokeSong(id) {
 ...
 const item = await getLibraryItemById(id);
 if (!item) {
 alert(" No se encontró el karaoke"); ⚠
 return;
 }
 const payload = {
 app: "SingIt",
...
async function importKaraokeFile(file) {
 ...
 await loadMyKaraokeSongs();
 await renderLibrary("todos");
 alert(` "${data.name}" importado en la Biblioteca y en Karaoke → Mis ✅
Canciones`);
 } catch (err) {
...
function cambiarEscenarioKaraoke() {
 ...
 const todosLosTemas = ["theme-clasico", "theme-moderno", "theme-disco",
"theme-acustico", "theme-fiesta", "theme-retrowave"];
...
// ==========================================
document.getElementById("karaokeThemeSelect")?.addEventListener("change",
