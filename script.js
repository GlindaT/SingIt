...
window.webkitAudioContext)();
 ...
 karaokeStream = await navigator.mediaDevices.getUserMedia({ audio:
audioConstraints1 });
 ...
 karaokeStream2 = await navigator.mediaDevices.getUserMedia({ audio:
audioConstraints2 });
 // Procesar Mic 2 con tus filtros
 const source2 =
karaokeDuoAudioContext.createMediaStreamSource(karaokeStream2);
...
async function startKaraokePitchDetection() {
 ...
 // Le damos una oportunidad al sistema de inicializar en el primer frame del
loop.
 function loop() {
 const track = $("karaokeTrack");
 const currentTime = track ? track.currentTime : 0;
 // --- PROTECCIÓN PARA EL MICRÓFONO 1 ---
 // Si el analizador 1 ya está listo, configuramos su tamaño y leemos los
datos
 let pitch1 = -1;
...
48000);
 ...
 // ENVIAR AMBOS TONOS AL MONITOR VISUAL (Solo si al menos el monitor
existe)
...
function cambiarEscenarioKaraoke() {
 ...
 // Si el select está vacío momentáneamente por la carga, usamos el clásico por
defecto
 if (!nuevoTema) {
 return;
 }
